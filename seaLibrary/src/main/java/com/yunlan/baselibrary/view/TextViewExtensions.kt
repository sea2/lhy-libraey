...
class TextViewSuffixWrapper(val textView: TextView) {
    private data class SuffixColor(val fromIndex: Int, val toIndex: Int, val color: Int?)

    var mainContent: SpannableStringBuilder = textView.text as SpannableStringBuilder
        set(value) {
            collapseCache = null
            field = value
        }
    ...
    fun suffixColor(fromIndex: Int, toIndex: Int, listener: View.OnClickListener) {
        suffixColorList.add(SuffixColor(fromIndex, toIndex, null))
    }
    ...
    private fun performCollapse(transition: Transition?) {
        ...
        fun defaultCollapse() {
            ...
            textView.setText(mainContent, TextView.BufferType.SPANNABLE);
            if (transition != null) {
                textView.apply {
